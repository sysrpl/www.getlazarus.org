...
function initContent() {
	...
	try {
		...
		console.log("error detected in guide section");
	}
	// tooltipster plugin
	$(".tooltip").tooltipster({
...
function initGuides() {
	...
	function lineMouseOver(e) {
		let guide = e.currentTarget;
		if (activeGuide == guide)
			return;
		if (activeGuide != null)
			for (let i = 0; i < activeGuide.lines.length; i++)
				activeGuide.lines[i].classList.remove("guide-line");
		activeGuide = guide;
		for (let i = 0; i < guide.lines.length; i++)
			guide.lines[i].classList.add("guide-line");
	}
	...
	function lineMouseOut(e) {
		...
		let guide = e.currentTarget;
		for (let i = 0; i < guide.lines.length; i++)
			guide.lines[i].classList.remove("guide-line");
	}

	let guides = document.querySelectorAll(".guide");
	for (let i = 0; i < guides.length; i++) {
		let guide = guides[i];
		let firstLine = parseInt(guide.parentElement.querySelectorAll(".syntaxhighlighter .lines .line .number")[0].innerText);
		guide.classList.add("tooltip");
		guide.lines = [];
		let lines = guide.getAttribute("lines").split(" ");
		if (lines.length == 1) {
			let range = guide.getAttribute("lines").split("-");
			if (range.length == 2) {
				lines = [];
				for (let j = parseInt(range[0]); j < parseInt(range[1]) + 1; j++)
					lines.push(j);
			}
		}
		for (let j = 0; j < lines.length; j++) {
			let index = parseInt(lines[j]) - firstLine;
			let line = guide.parentElement.querySelectorAll(".syntaxhighlighter .lines .line .content .block")[index];
			if (guide.getAttribute("title") == null) {
				let title = `<b>line ${index + firstLine}:</b><span class="tip-line">${line.innerText.trim()}</span>`;
				guide.setAttribute("title", title);
			}
			guide.lines.push(line);
		}
		guide.addEventListener("mouseover", lineMouseOver);
		guide.addEventListener("mouseout", lineMouseOut);
	}
}
