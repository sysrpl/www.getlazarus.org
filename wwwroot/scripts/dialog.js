...
var dialogDefaultEasing = "rubber";
var dialogDefaultTheme = "lazarus";
...
function dialogShow(dialog) {
	...
	var animation = function() {
		if (currentDialog.animate) {
			...
			dialogWindow.css({marginTop: -100}).animate({marginTop: 0}, 500, easing);
		}
		else
...
function dialogHide() {
	if (dialogBusy())
		return;
	if (currentDialog)
		$("#dialogBackground").fadeOut(dialogClear);
	dialogDestroy();
}
...
function dialogTest() {
	var dialog = {
		...
		content: 'In linguistics, a yes–no question, formally known as a polar question, is a question whose expected answer is either "yes" or "no". Formally, they present an exclusive disjunction, a pair of alternatives of which only one is acceptable. ',
		showClose: true,
		accept: "Yes",
...
function dialogInit() {
	...
	dialogBackground.innerHTML = '<img id="dialogBusy" src="http://cache.dotaplayer.info/images/busy.gif"><div id="dialogContainer">' +
